...
public:
	...
	void insert(Datos datos)
	{
		...
		if (tipo == 1)
		{
			while (i < size)
			{
				index = hashing_method(datos, size, i, tipo);
				if (table[index].userId == -1)
				{
					table[index] = datos;
					return;
				}
		...
		cout << "Error: la tabla hash está llena, no se puede insertar el elemento." << endl;
	}
	...
	bool search(Datos datos)
	{
		...
		{
			cout << "Error al reconocer tipo de dato" << endl;
			return false; // Asegurar que siempre se devuelva un valor
		}
	}
};
...
int main(int argc, char const *argv[])
{
	...
	HashTable ht_linear_ID(40000, 1, linear_probing);
	HashTable ht_linear_Name(40000, 2, linear_probing);

	HashTable ht_quadratic_ID(40000, 1, quadratic_probing);
	HashTable ht_quadratic_Name(40000, 2, quadratic_probing);

	HashTable ht_double_ID(40000, 1, double_hashing);
	HashTable ht_double_Name(40000, 2, double_hashing);
	...
	for (int experimento = 1; experimento <= 20; ++experimento)
	{
		...
		for (int i = 0; i < cantElem; i++)
		{
			...
			cout << experimento << ";universities_followers;linear_probing;ID;" << cantElem << ";" << durationID.count() << endl;
			...
			cout << experimento << ";universities_followers;linear_probing;Name;" << cantElem << ";" << durationNAME.count() << endl;
		}
	}
	...
	for (int experimento = 1; experimento <= 20; ++experimento)
	{
		...
		for (int i = 0; i < cantElem; i++)
		{
			...
			// Calculo de tiempo de inserción: PRUEBA PARA ID
			auto startID = chrono::high_resolution_clock::now();
			ht_quadratic_ID.insert(datos);
			auto endID = chrono::high_resolution_clock::now();
			auto durationID = chrono::duration_cast<chrono::nanoseconds>(endID - startID);
			cout << experimento << ";universities_followers;quadratic_probing;ID;" << cantElem << ";" << durationID.count() << endl;

			// Calculo de tiempo de inserción: PRUEBA PARA NAME
			auto startNAME = chrono::high_resolution_clock::now();
			ht_quadratic_Name.insert(datos);
			auto endNAME = chrono::high_resolution_clock::now();
			auto durationNAME = chrono::duration_cast<chrono::nanoseconds>(endNAME - startNAME);
			cout << experimento << ";universities_followers;quadratic_probing;Name;" << cantElem << ";" << durationNAME.count() << endl;
		}
	}
	...
	for (int experimento = 1; experimento <= 20; ++experimento)
	{
		...
		for (int i = 0; i < cantElem; i++)
		{
			...
			// Calculo de tiempo de inserción: PRUEBA PARA ID
			auto startID = chrono::high_resolution_clock::now();
			ht_double_ID.insert(datos);
			auto endID = chrono::high_resolution_clock::now();
			auto durationID = chrono::duration_cast<chrono::nanoseconds>(endID - startID);
			cout << experimento << ";universities_followers;double_hashing;ID;" << cantElem << ";" << durationID.count() << endl;

			// Calculo de tiempo de inserción: PRUEBA PARA NAME
			auto startNAME = chrono::high_resolution_clock::now();
			ht_double_Name.insert(datos);
			auto endNAME = chrono::high_resolution_clock::now();
			auto durationNAME = chrono::duration_cast<chrono::nanoseconds>(endNAME - startNAME);
			cout << experimento << ";universities_followers;double_hashing;Name;" << cantElem << ";" << durationNAME.count() << endl;
		}
	}
	file.close();
